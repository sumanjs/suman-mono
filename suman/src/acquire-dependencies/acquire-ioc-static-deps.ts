...
export const acquireIocStaticDeps = function () {
  ...
  try {
    ...
    return iocPromise = Promise.resolve(_suman.$staticIoc = Object.freeze({}));
  }
  ...
  const promises = Object.keys(ret).map(function (key) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (typeof fn !== 'function') {
        ...
        reject(new Error(
          chalk.red(
            ' => Suman usage error => <suman.ioc.js> functions take 0 or 1 arguments, ' +
  ...
  return iocPromise = Promise.all(promises).then(function (deps) {
      ...
      return _suman.$staticIoc = Object.freeze(final);
      
    },
    function (err) {
      _suman.log.error(err.stack || err);
      _suman.log.error('despite the error, suman will continue optimistically.');
      return _suman.$staticIoc = Object.freeze({});
    });
